...
void pwm_init(void){
	...
	GPIOA->MODER |= (GPIO_MODER_ALTARNATE << GPIO_MODER_MODE8_Pos) | (GPIO_MODER_ALTARNATE << GPIO_MODER_MODE9_Pos);
	GPIOA->MODER |= (GPIO_MODER_OUTPUT << GPIO_MODER_MODE0_Pos) | (GPIO_MODER_OUTPUT << GPIO_MODER_MODE3_Pos);
	GPIOA->AFR[1] |= GPIO_AFRL_AFRL0_0 | GPIO_AFRL_AFRL1_0;	
	GPIOA->ODR |= GPIO_ODR_OD0 | GPIO_ODR_OD3;
	//Set TIM1 parameters 
